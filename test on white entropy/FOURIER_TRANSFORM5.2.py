...
################################################################################################
# INPUT PARAMETERS #
nameout= "SMOOTHED_WHITE_kappa_0.01.txt" #sys.argv[1]      #name output file for gap statistics
namein= "random_signal_" #"Log" #sys.argv[2]       #name inputfiles without iterator
start = 1 #int(sys.argv[3]) #start index ->1
end = 15 #int(sys.argv[4])+1 #end index ->13 !!!! 27 !!!!
kappa= 0.01 #float(sys.argv[5]) #smoothing parameter of Kernel function -> kappa large strong smoothing and vice versa (SPECTRAL SMOOTHING)
...
SavitzkyINT=0   # smoothing of data using a Savitzky-Golay filter (alternative to Kernel smoothing)
